...
**对应实操**：4.3节nuScenes轨迹预测展示真实交通场景中的时序建模。
...
本章通过四个递进式案例，覆盖智能驾驶核心技术链路：

1. **案例一：行为克隆** - 端到端学习，图像直接映射转向角，理解模仿学习范式
2. **案例二：世界模型（轨迹预测）** - 基于历史轨迹预测未来运动，掌握时序建模与状态转移
3. **案例三：BEV空间表示** - 透视图转俯视图，体验结构化感知的度量空间优势
4. **案例四：VLA决策** - 视觉-语言-动作融合，探索多模态决策的前沿方向


### 4.1 你将学到什么

本节完成“行为克隆”从数据准备、训练到评估的完整闭环，  
对应智能驾驶的最小端到端落地实操。
...
#### 4.2.5 训练问题诊断实战（对应1.3.3节）

**本案例可能遇到的训练问题及诊断流程**：

根据**1.3.3节训练问题诊断表**，行为克隆任务容易出现以下问题：

| 实际问题 | 现象描述 | 诊断表对应行 | 处置方案（按优先级） | 代码位置 |
|---------|---------|-------------|---------------------|---------|
| **Loss震荡不稳定** | 相邻epoch的loss波动>20%<br>曲线呈锯齿状 | Loss震荡/不稳定 | ✅ **lr降至0.5倍**（1e-3→5e-4）<br>✅ 检查转向角是否归一化<br>加BatchNorm在卷积后 | `optimizer = Adam(lr=5e-4)`<br>`steering = (steering - mean) / std` |
| **过拟合严重** | train_loss下降但val_loss上升<br>gap>30% | 过拟合严重 | ✅ **数据增强**：加RandomBrightness、Shadow<br>✅ **L2正则化**：`weight_decay=1e-4`<br>加Dropout(0.3)在FC层前 | `optimizer = Adam(..., weight_decay=1e-4)`<br>`transforms.ColorJitter(brightness=0.2)` |
| **收敛太慢** | 10个epoch后MSE降幅<5% | 收敛太慢 | ✅ **lr增至2倍**（1e-4→2e-4）<br>检查图像预处理是否丢失关键特征<br>考虑用预训练模型（ResNet18） | `optimizer = Adam(lr=2e-4)`<br>替换backbone为`resnet18(pretrained=True)` |
| **预测全部偏向0** | 90%预测值在[-0.05, 0.05]<br>转向角分布严重不均 | 测试集完全失效 | ✅ **数据平衡**：对大转向角样本过采样<br>✅ **损失函数加权**：大角度loss权重×2<br>检查是否需要从回归改为分类 | `weights = torch.where(abs(y)>0.2, 2.0, 1.0)`<br>`loss = (mse * weights).mean()` |

**实操建议**：
1. **首先运行baseline**：不做任何修改，观察loss曲线和MSE
2. **遇到问题再诊断**：对照上表找到对应行，按优先级尝试
3. **记录每次调整**：修改前后的loss对比，写入训练日志
4. **避免过度调参**：每次只改一个参数，观察2-3个epoch效果

**与第1章理论的关联**：
- 诊断表是**通用方法论**（适用所有深度学习任务）
- 本节是**具体应用实例**（行为克隆的特定问题场景）
...
### 4.3 案例二：世界模型（nuScenes 轨迹预测）

#### 4.3.1 真实场景中的轨迹预测

**世界模型的核心任务**：从当前状态预测未来状态（s_t → s_{t+1}）。在智能驾驶中，这意味着预测车辆、行人的未来位置，这是规划决策的关键输入。

**为什么选择 nuScenes 轨迹预测？**

| 维度 | nuScenes 轨迹预测 | 像素级视频预测 |
|------|------------------|---------------|
| **数据类型** | 目标轨迹坐标（x,y,v,θ） | 高分辨率图像序列 |
| **训练时间** | 10-20分钟（CPU可行） | 数小时（需大量GPU） |
| **工业应用** | ✅ Tesla/Waymo核心技术 | ⚠️ 计算成本高，实用性有限 |
| **实战价值** | ✅ 直接应用于规划决策 | ⚠️ 难以提取可用决策信号 |
| **可解释性** | ✅ 轨迹空间清晰（米/秒） | ⚠️ 像素空间难以解释 |
| **数据需求** | nuScenes mini（850场景） | 需大规模视频数据 |

**nuScenes 轨迹预测的价值**：
- **工业级场景**：与 Waymo Open Motion Dataset、Argoverse 同类任务
- **真实交通语义**：包含车辆交互、变道、转弯等复杂行为
- **结构化表示**：输出轨迹坐标，直接用于规划与控制
- **能力对标**：Tesla FSD、Waymo Driver 的核心预测模块

**与行为克隆的区别**：
- 行为克隆：学习"我"该如何行动（图像→转向角）
- 轨迹预测：预测"他人"将如何行动（历史轨迹→未来轨迹）
- 预测是决策的前置依赖：先知道周围车会去哪，才能规划自己的路径

#### 4.3.2 数据准备与特征工程

**教学数据说明**：

为了在课堂上快速演示轨迹预测，本案例使用**合成数据**模拟 nuScenes 真实场景特征。合成数据包含三种典型运动模式：
1. **直行**：匀速或匀加速前进
2. **变道**：S型横向位移 + 纵向前进  
3. **转弯**：圆弧轨迹

合成数据的优势：
- ✅ 无需下载 4GB nuScenes 数据集
- ✅ 训练时间短（10-15分钟 vs 数小时）
- ✅ 核心算法与真实数据完全一致
- ✅ 可自定义场景难度和数量
...
# 3. 提取轨迹特征（需单独脚本，此处省略）
# python tutorial_runs/extract_nuscenes_trajectories.py \
#     --dataroot ./v1.0-mini \
#     --output ./tutorial_runs/data/nuscenes_trajectories.csv
```

**轨迹预测任务定义**：
- **输入（历史）**：目标在过去 2 秒的轨迹（20 帧 @ 10Hz）
  - 特征：[x, y, vx, vy, heading]（5维向量 × 20帧）
  - x, y：BEV坐标（米），相对于自车
  - vx, vy：速度分量（米/秒）
  - heading：航向角（弧度）
  
- **输出（未来）**：目标在未来 3 秒的轨迹（30 帧）
  - 预测：[x, y]（2维坐标 × 30帧）

**数据统计**：
- 合成轨迹样本：6,000 条
- 训练/验证/测试：70% / 15% / 15%（4200 / 900 / 900）
- 数据生成时间：< 5 秒

**数据可视化预览**：

为帮助理解轨迹数据的特征，以下可视化展示了合成数据集的关键属性：

1. **典型运动模式示例**  
   ![Trajectory Patterns](tutorial_runs/output/trajectory_patterns.png)  
   展示三种典型运动模式：直行（匀速）、变道（S型）、转弯（圆弧），每种模式包含历史轨迹（绿色）和未来轨迹（蓝色）。

2. **轨迹数据分布分析**  
   ![Trajectory Distribution](tutorial_runs/output/trajectory_distribution.png)  
   - 左图：轨迹长度分布直方图
   - 中图：速度分布统计（5-15 m/s）
   - 右图：运动模式占比（直行/变道/转弯）

3. **BEV俯视图轨迹示例**  
   ![BEV Trajectories](tutorial_runs/output/bev_trajectory_samples.png)  
   BEV视角下的6个轨迹样本，展示相对于自车（原点）的他车运动轨迹，包含历史（实线）和未来（虚线）。

4. **轨迹预测动态演示（驾驶者视角GIF）**  
   ![Trajectory Driver View Demo](tutorial_runs/output/trajectory_driver_view_demo.gif)  
   **驾驶者主视角**展示前方车辆轨迹预测过程：
   - **道路场景**：透视效果的车道线和路面，模拟真实驾驶视野
   - **蓝色车辆**：前方目标车辆的真实位置
   - **红色车辆**：模型对前方车辆的预测位置
   - **黄色虚线**：预测的行驶轨迹投影在路面上
   - **信息面板**：实时显示ADE/FDE误差和预测状态
   - **演示场景**：前方车辆变道场景（20帧历史 + 30帧预测）
   - GIF包含60帧，展示6秒完整预测过程，自动循环播放

**可视化脚本**：
- 静态图表生成：训练脚本 `nuscenes_trajectory_train.py` 自动生成
- 驾驶者视角GIF：`tutorial_runs/create_driver_view_demo.py`

**与行为克隆数据的对比**：

| 维度 | 行为克隆 | 轨迹预测 |
|------|---------|---------|
| **数据类型** | 自车图像 + 转向角 | 他车轨迹坐标序列 |
| **时序长度** | 单帧（无时序） | 50 帧（5秒时间窗） |
| **标注成本** | 低（驾驶记录自带） | 高（需人工跟踪标注） |
| **物理意义** | 控制信号 | 空间位置（米） |
| **演示动画** | 车载相机视角行驶过程 | 驾驶者视角前车预测 |
...
```python
# 方案一：LSTM 编码器-解码器（baseline）
Encoder: LSTM(input_size=5, hidden_size=128, num_layers=2)
Decoder: LSTM(hidden_size=128) + Linear(128 -> 2)

# 方案二：Transformer（进阶方案，可选）
Encoder: Multi-head Attention (d_model=128, n_heads=4)
Decoder: Autoregressive generation with attention
```

**关键设计思想**：
- **历史编码**：将 20 帧历史信息编码为固定维度隐状态
- **时序建模**：LSTM/Transformer 捕捉运动动态模式
- **未来生成**：自回归方式逐帧预测未来 30 帧坐标
- **物理约束**：可选加入速度/加速度约束（提升合理性）
...
- **损失函数**：MSE（预测坐标与真实坐标的均方误差）
- **训练时间**：CPU 约 10-15 分钟

**训练命令**：

```bash
# LSTM轨迹预测器（使用合成数据模拟 nuScenes）
python tutorial_runs/nuscenes_trajectory_train.py --epochs 30

# 自定义参数
python tutorial_runs/nuscenes_trajectory_train.py \
    --epochs 30 \
    --batch_size 64 \
    --num_samples 6000 \
    --hidden_size 64
...
**输出文件**：
- `tutorial_runs/output/nuscenes_trajectory_model.pth`（模型权重）
- `tutorial_runs/output/nuscenes_train_log.csv`（训练日志）
- `tutorial_runs/output/nuscenes_loss_curve.png`（损失曲线图）
- `tutorial_runs/output/nuscenes_predictions_viz.png`（轨迹预测可视化）
- `tutorial_runs/output/nuscenes_trajectory_metrics.json`（评估指标摘要）

**损失函数细节**：

```python
# 基础 MSE loss
loss = MSE(predicted_xy, ground_truth_xy)  # 形状: [batch, 30, 2]

# 可选：时间加权 loss（远期预测权重更高）
time_weights = torch.linspace(1.0, 2.0, 30)  # 1秒权重1.0，3秒权重2.0
loss = (MSE_per_frame * time_weights).mean()

# 可选：多模态预测（生成多条可能轨迹）
# 输出 K=6 条候选轨迹，取最优轨迹计算 loss
```
...
ADE = mean(||predicted_xy - ground_truth_xy||_2)  # 单位：米
# 业界基准：ADE < 1.5m（优秀），< 2.5m（可接受）

# 2. FDE (Final Displacement Error) - 终点位移误差
FDE = ||predicted_xy[-1] - ground_truth_xy[-1]||_2  # 单位：米
# 关注预测终点的准确性（对规划最重要）

# 3. Miss Rate - 预测失败率
Miss_Rate = percentage(FDE > 2.0m)  # 终点误差超过2米的比例
# 业界基准：Miss Rate < 30%
```
...
```
Epoch  1:  train_ADE=15.32m, val_ADE=14.87m, val_FDE=26.34m, Miss_Rate=95.2%
Epoch 10:  train_ADE=2.15m,  val_ADE=2.48m,  val_FDE=4.12m,  Miss_Rate=52.3%
Epoch 20:  train_ADE=1.38m,  val_ADE=1.72m,  val_FDE=2.95m,  Miss_Rate=31.5%
Epoch 30:  train_ADE=1.08m,  val_ADE=1.45m,  val_FDE=2.38m,  Miss_Rate=24.7%

测试集最终结果：
Test ADE: 1.42m, Test FDE: 2.31m, Miss Rate: 25.1%
```

**损失曲线分析**：
- **训练曲线快速下降**：前 10 轮从 2.5m 降至 1.2m
- **验证曲线平稳收敛**：30 轮后趋于稳定
- **泛化能力评估**：val_ADE 与 train_ADE 差距约 0.5-0.8m（正常）

**训练过程可视化**：

1. **损失曲线图（三合一）**  
   ![Loss Curves](tutorial_runs/output/nuscenes_loss_curve.png)  
   - 左图：训练损失 vs 验证损失对比
   - 中图：ADE 和 FDE 指标随训练轮数变化
   - 右图：Miss Rate 下降趋势

2. **轨迹预测可视化结果**  
   ![Prediction Visualization](tutorial_runs/output/nuscenes_predictions_viz.png)  
   4个典型测试样本的BEV预测结果：
   - 绿色实线：历史轨迹（过去2秒）
   - 蓝色实线：真实未来轨迹（ground truth）
   - 红色虚线：模型预测轨迹
   - 每个子图标注 ADE 和 FDE 误差值

**BEV 轨迹可视化**：

```python
# 可视化内容（俯视图，以自车为原点）：
# 1. 历史轨迹（绿色）：过去 2 秒的真实运动
# 2. 真实未来（蓝色）：ground truth 未来 3 秒轨迹
# 3. 预测未来（红色）：模型预测的未来 3 秒轨迹
# 4. 误差标注：显示 ADE 和 FDE 数值

# 示例场景：
# - 直行车辆：预测准确，ADE < 1.0m
# - 变道车辆：初期准确，后期误差增大（ADE ≈ 2.0m）
# - 转弯车辆：预测难度高，ADE ≈ 2.5m
```
...
| 错误类型 | 表现 | 原因 | 改进方向 |
|---------|------|------|---------|
| **直线漂移** | 预测轨迹缓慢偏离真实轨迹 | 速度估计不准 | 加入速度损失项 |
| **变道延迟** | 预测变道意图晚于真实 | 缺少意图识别 | 引入注意力机制 |
| **轨迹震荡** | 预测轨迹抖动 | 模型不稳定 | 增加平滑约束 |
| **交互忽略** | 未考虑他车影响 | 单体预测 | 引入图网络（GNN） |

**预测误差分析可视化**：

3. **误差分布统计**  
   ![Error Distribution](tutorial_runs/output/trajectory_error_distribution.png)  
   - 左图：ADE误差直方图（测试集900个样本）
   - 右图：FDE误差直方图，标注2m阈值线（Miss Rate判定标准）

4. **不同运动模式的预测精度对比**  
   ![Mode Performance](tutorial_runs/output/trajectory_mode_performance.png)  
   对比直行、变道、转弯三种模式的ADE/FDE/Miss Rate，显示不同场景的预测难度差异。

**可视化工具**：

所有可视化图表在训练脚本 `nuscenes_trajectory_train.py` 运行时自动生成，包括：
- 数据分布分析（训练前生成）
- 损失曲线（训练过程实时更新）
- 预测结果对比（训练后生成）
- 误差统计分析（测试集评估时生成）
...
```bash
# 生成交互式可视化（HTML 格式，可选）
python tutorial_runs/visualize_trajectory_predictions.py \
    --model tutorial_runs/output/nuscenes_trajectory_model.pth \
    --output tutorial_runs/output/trajectory_viz.html
```
...
| 维度 | 行为克隆 | 轨迹预测 |
|------|---------|---------|
| **输入可视化** | 驾驶图像+转向角度叠加 | BEV轨迹序列（历史+未来） |
| **输出可视化** | 转向角度时序图 | BEV空间轨迹曲线 |
| **评估指标图** | MSE损失曲线 | ADE/FDE/Miss Rate多指标 |
| **动态展示** | 驾驶场景连续帧GIF | BEV轨迹演化动画GIF |
| **空间维度** | 2D图像（像素） | 2D空间（米） |
| **动画特点** | 摄像头视角+UI叠加 | 俯视图+多轨迹对比 |

#### 4.3.5 训练问题诊断实战（对应1.3.3节）

**本案例可能遇到的训练问题及诊断流程**：

根据**1.3.3节训练问题诊断表**，轨迹预测任务容易出现以下问题：

| 实际问题 | 现象描述 | 诊断表对应行 | 处置方案（按优先级） | 代码位置 |
|---------|---------|-------------|---------------------|---------|
| **ADE/FDE不下降** | 5个epoch后ADE变化<0.1m | Loss不下降/不收敛 | ✅ **lr降至0.1倍**（1e-3→1e-4）<br>✅ 增加LSTM hidden_size（64→128）<br>检查轨迹坐标是否归一化 | `optimizer = Adam(lr=1e-4)`<br>`hidden_size=128` |
| **预测轨迹漂移** | 长时预测（>2s）时轨迹严重偏离<br>FDE远大于ADE | 过拟合严重 | ✅ **Teacher Forcing比例调整**：训练时70%用真值<br>✅ **L2正则化**：`weight_decay=1e-4`<br>增加轨迹噪声增强 | `teacher_forcing_ratio=0.7`<br>`noise = np.random.normal(0, 0.1, shape)` |
| **Miss Rate过高** | >50%预测与真值距离>2m | 收敛太慢 | ✅ **多模态预测**：输出K=3条候选轨迹<br>✅ 增加注意力机制关注历史关键帧<br>lr增至2倍 | `output = model(x, num_modes=3)`<br>添加`MultiHeadAttention`模块 |
| **某类运动模式失效** | 直行正常但变道ADE>5m | 欠拟合 | ✅ **数据平衡**：变道样本过采样至50%<br>✅ **增加模型容量**：LSTM层数+1<br>分模式训练独立模型 | `oversample_lane_change(trajectories)`<br>`num_layers=3` |
| **训练速度极慢** | 单epoch>10分钟（6000样本） | 训练时间太长 | ✅ **batch_size增大**（16→64）<br>✅ 减少未来预测帧数（30→20帧）<br>使用DataLoader多进程 | `batch_size=64`<br>`future_frames=20`<br>`num_workers=4` |
| **Loss出现NaN** | 训练中断显示nan | NaN/Inf出现 | ✅ **梯度裁剪**：`clip_grad_norm_(max_norm=1.0)`<br>✅ **lr降至1e-5**<br>检查轨迹数据是否包含Inf值 | `torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)` |

**轨迹预测特有的调试技巧**：
1. **可视化预测轨迹**：不要只看数值，画出BEV图观察形状是否合理
2. **分阶段检查**：短期预测（0.5s）、中期（1.5s）、长期（3s）分别评估
3. **分运动模式统计**：直行/变道/转弯分别计算ADE，定位薄弱环节
4. **检查历史依赖**：遮挡部分历史帧，测试模型是否过度记忆

**与第1章理论的关联**：
- **时序建模特殊性**：LSTM的梯度问题比CNN更常见，需要特别注意梯度裁剪
- **回归任务特点**：Loss不下降时，优先检查归一化和学习率，而非模型结构
- **长时预测挑战**：误差累积是轨迹预测的核心难题，需要Teacher Forcing等特殊技巧

#### 4.3.6 与 3.2 理论的对应关系

**理论映射**：

| 3.2 理论概念 | nuScenes 轨迹预测实现 |
|-------------|---------------------|
| **状态 s_t** | 目标在 t 时刻的位置和速度 [x,y,vx,vy,θ] |
| **状态转移 s_t → s_{t+1}** | LSTM/Transformer 编码历史 → 预测未来坐标 |
| **时序建模** | LSTM/Attention 捕捉车辆运动动态 |
| **预测未来** | 输出未来 3 秒（30 帧）轨迹坐标 |
| **世界模型目标** | 最小化 ADE/FDE（预测误差） |

**从教学案例到工业实践**：

| 维度 | nuScenes 轨迹预测 | 工业级方案（Tesla/Waymo） |
|------|------------------|------------------------|
| **输入数据** | CSV 轨迹坐标 | BEV 特征 + 地图 + 交互 |
| **模型复杂度** | LSTM/Transformer | Multi-agent GNN + Attention |
| **预测模式** | 单模态（1条轨迹） | 多模态（6条候选轨迹+概率） |
| **时间跨度** | 3 秒 | 6-8 秒 |
| **评估指标** | ADE < 1.5m | ADE < 1.0m, Miss Rate < 20% |
| **训练数据量** | 15k 轨迹 | 数百万轨迹 |
| **工程部署** | 离线评估 | 实时推理（10Hz） |

**核心技术进阶方向**：

1. **多模态预测**（Multiple Futures）
   - 问题：同一历史可能有多种合理未来（变道 vs 直行）
   - 解决：输出 K=6 条候选轨迹 + 置信度
   - 代表：Waymo MultiPath, Uber MultiPath++

2. **社交交互建模**（Social Interaction）
   - 问题：车辆间相互影响（礼让、抢行）
   - 解决：图神经网络（GNN）建模车辆关系
   - 代表：TNT (VectorNet), LaneGCN

3. **地图融合**（Map Integration）
   - 问题：轨迹需遵循车道拓扑约束
   - 解决：将高精地图编码为向量/栅格特征
   - 代表：VectorNet, HOME

4. **不确定性估计**（Uncertainty Quantification）
   - 问题：预测置信度量化（异常行为检测）
   - 解决：贝叶斯网络、集成学习
   - 代表：Trajectron++, ProphNet

**检查点（世界模型案例完成）**：
- ✅ 理解轨迹预测在智能驾驶中的核心价值
- ✅ 成功训练 nuScenes 轨迹预测模型
- ✅ 掌握 ADE/FDE 评估指标的含义
- ✅ 能解读 BEV 可视化结果（历史、真实、预测）
- ✅ 理解与工业级方案的差距与进阶方向
- ✅ 能解释与 3.2 理论的对应关系

**对应实操**：参见本章 4.3 节完整实操流程。
...
说明：超过 100MB 的数据文件不上传 GitHub，请按以下方式自行下载并放入指定目录。

### CIFAR-10（用于第 2 章）
- 下载脚本（推荐）：运行 `tutorial_runs/cifar_cnn_resnet.py` 会自动下载 CIFAR-10
- 手动下载链接：https://www.cs.toronto.edu/~kriz/cifar-10-python.tar.gz
- 存储目录：`tutorial_runs/data/`（脚本会自动解压至 `tutorial_runs/data/cifar-10-batches-py/`）

### 行为克隆数据（用于第 4 章）
- 下载脚本：`python tutorial_runs/behavioral_cloning_download.py`
- 数据来源（公开镜像）：https://github.com/KansaiUser/BehavioralCloningTrackData
- 存储目录：`tutorial_runs/behavioral_cloning_data/`

### nuScenes mini（可选拓展）
- 官方下载页（需注册）：https://www.nuscenes.org/
- 示例命令（Linux/macOS）：`wget https://www.nuscenes.org/data/v1.0-mini.tgz`
- 存储建议：`tutorial_runs/data/`（自行解压为 `tutorial_runs/data/nuscenes/`）
